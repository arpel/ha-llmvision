        ...
        if False:
            ...
            try:
                async with aiosqlite.connect(self._db_path) as db:
                    await db.execute(
                        """
                        UPDATE events SET key_frame = REPLACE(key_frame, '/media/local/llmvision/snapshots', '/config/media/llmvision/snapshots')
                    """
                    )
                    await db.commit()
            except aiosqlite.Error as e:
                _LOGGER.error(f"Error migrating image paths in events.db (custom one): {e}")
